...
Moving into developing a Version 5, we maintained several goals in mind: shrinking the footprint, increasing speed and reliability, and maintaining or improving functionality. The physical box is significantly smaller than its predecessor, reducing by an overall 11 square centimeters. Our improved platform has yet to be tested firmware-wise, but we hope the new microcontroller will significantly improve response times and state detection accuracy. Our modular approach to design and implementation has allowed for increased functionality and the previous design with a slot and button is still available if users so wish.
</details>
...
The Portalbox has been designed to as closely as possible comply with IPC-2221 for General Design, and more specifically for IPC-2222 Rigid design specifications. Please make us aware of any discrepancies that may come up.
</details>
